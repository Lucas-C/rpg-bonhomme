...
curl -s http://localhost:$PORT/jsonp_db/urlencoded_dict?callback=foo | grep 'foo({name:"John Doe"}, null)'
...
# Error handling:
curl -s http://localhost:$PORT/jsonp_db | grep 'HTTPError: Incorrect request syntax'
curl -s http://localhost:$PORT/jsonp_db/a/ | grep 'HTTPError: Incorrect request syntax'
...
#curl -s http://localhost:$PORT/jsonp_db/nonascii?aéè | grep 'àéè'
# TODO FIXME
...
curl -s "http://localhost:$PORT/jsonp_db/K?V2&modification-key=DUMMY" | grep 'HTTPError: Invalid modification-key'
curl -s http://localhost:$PORT/jsonp_db/K?V2 | grep 'HTTPError: No modification-key provided'
